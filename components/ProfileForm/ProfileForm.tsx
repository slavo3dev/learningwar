...
export function ProfileForm({ profile }: { profile: Profile }) {
	...
	return (
		<form
			ref={formRef}
			action={handleSubmit}
			className='max-w-lg space-y-4 rounded-xl border border-gray-200 bg-white p-6 shadow-sm'
		>
			<div>
				...
				<p className='mt-1 text-xs text-gray-400'>
					Shown on your porch posts instead of your username. Leave blank to show your username.
				</p>
			</div>
			...
			<label className='flex items-start gap-2 text-sm text-gray-600'>
				...
				/>
				<span>
					Share my email on my porch posts, so other learners can connect with me
				</span>
			</label>
